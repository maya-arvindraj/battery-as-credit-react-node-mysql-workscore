 ...
 * WorkScore response
 *
 * The original WorkScore fields are kept unchanged.
 *
 * The ML fields below are optional because the backend
 * does not return them yet. They will be populated after
 * we connect the Node backend to the Flask ML service.
 */
export type WorkScore = {
  // Existing rule-based WorkScore
  score: number
  status: string

  // Existing WorkScore breakdown
  earnings_stability: number
  delivery_consistency: number
  work_days: number
  repayment_history: number
  ev_usage_tenure: number
  weights: Record<string, number>

  // Existing rider metrics
  deliveries: number
  monthly_earnings: number
  working_days: number
  payment_reliability: number
  ev_usage_score: number

  /*
   * ML repayment prediction
   *
   * These values come from the Random Forest model
   * running in the separate Flask ML service.
   *
   * They are optional for now because ML integration
   * has not yet been connected to the backend.
   */
  ml_prediction?: number
  ml_prediction_label?: string
...
export function token() {
  return localStorage.getItem('bac_token')
}

export function user(): User | null {
  try {
    return JSON.parse(localStorage.getItem('bac_user') || 'null')
  } catch {
    return null
  }
}

export function setAuth(access_token: string, u: User) {
  localStorage.setItem('bac_token', access_token)
  localStorage.setItem('bac_user', JSON.stringify(u))
}
...
export const api = {
   ...
   * WorkScore
   *
   * The response now supports both:
   * 1. Existing rule-based WorkScore
   * 2. ML repayment prediction
   */
  workscore: () =>
    request<WorkScore>('/api/workscore'),

  updateWorkscore: (data: {
    deliveries: number
    monthly_earnings: number
    working_days: number
    payment_reliability: number
  ...
  repayments: () =>
    request<Repayment[]>('/api/repayments'),

  payRepayment: (id: number) =>
    request<Repayment>(
      `/api/repayments/${id}/pay`,
      {
        method: 'POST'
      }
    ),
  ...
  ownership: () =>
    request<any>('/api/ownership'),

  lenderSummary: () =>
    request<any>('/api/lender/summary'),

  lenderApplications: () =>
    request<any[]>('/api/lender/applications'),

  lenderRiders: () =>
    request<any[]>('/api/lender/riders'),

  lenderApplication: (id: number) =>
    request<any>(
      `/api/lender/applications/${id}`
    ),
